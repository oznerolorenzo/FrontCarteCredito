...
export class NewRicaricaComponent implements OnInit {
  ...
  constructor(
    ...
    this.ricaricaForm = this.fb.group({
      ...
      importoRicarica: ['', [Validators.required, Validators.min(0)]]
    });
  }
  ...
  onSubmit() {
    if (this.ricaricaForm.valid) {
      ...
      newRicarica.dataRicarica = new Date(newRicarica.dataRicarica); // Assicurati che la data sia in un formato valido
